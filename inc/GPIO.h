...
/**
 * @brief Initialize Port 8 (P8) as GPIO output pins.
 *
 * This function configures Port 8 (P8) of the TI MSP432 LaunchPad as GPIO output pins.
 * Specifically, it configures the following pins as GPIO output pins: P8.0, P8.5, P8.6, and P8.7.
 * The front and back LEDs are mapped as follows:
 *  - Front Left Yellow LED     <-->  P8.0
 *  - Front Right Yellow LED    <-->  P8.5
 *  - Back Left Red LED         <-->  P8.6
 *  - Back Right Red LED        <-->  P8.7
 *
 * The initial state of these pins is set to logic low (0).
 *
 * @return None
 */
void P8_Init();
